...
impl State {
    fn from(input: &str, repeats: u32) -> Result<Self> {
        ...
        let groups = g.iter().flatten().copied().collect();
    ...
    fn is_group_size_below_remaining(&self) -> bool {
        let left =
            self.sequence.len() as i64 - self.groups.iter().sum::<i64>() - self.groups.len() as i64
                + 1;
        left < 0
    }
    ...
    fn find_next_combinations(&self, cache: &mut BTreeMap<State, i64>) -> i64 {
        ...
        let first = if &self.sequence[0..1] != "#" {
            let c = &self.sequence[1..].trim_start_matches('.');
            let n = State {
        ...
        {
            let c = &self.sequence[self.groups[0] as usize + 1..].trim_start_matches('.');
            let v = self.groups.iter().skip(1).copied().collect();
            let s = State {
                sequence: c.to_string(),
                groups: v,
            };
...
mod test {
    ...
    #[test]
    fn test_solutionspart_2() -> Result<()> {
        // Arrange
        let input = fs::read_to_string("../../data/day12_data_test.txt")?;
        for (expected, count) in vec![(525152, 5), (21, 1)] {
            // Act
            let combinations = find_combinations(&input, count)?;
