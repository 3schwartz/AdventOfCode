...
impl Game {
    pub fn new(input: &str, multiple: u32) -> Self {
        ...
        let players: u32 = split[0].parse().expect("not able to parse player");
        let marbles: u32 = split[6].parse().expect("not able to parse marbles");

        return Self {
            players,
            marbles: marbles * multiple,
        };
    }
    ...
    pub fn find_highest_score(&self) -> GenericResult<u128> {
        ...
        let mut marbles: HashMap<u32, Marble> = HashMap::from([(0, Marble::new(0, 0, 0))]);
        ...
        for round in 1..=self.marbles {
            ...
            if round % 23 == 0 {
                ...
                marbles.get_mut(&left).map(|m| m.right = right);
                marbles.get_mut(&right).map(|m| m.left = left);
