...
pub fn all_benchmark(c: &mut Criterion) {
    let input = std::fs::read_to_string("../../data/day5_data.txt").expect("couldn't open file");
    ...
    group.bench_function("Value - All", |b| {
        b.iter(|| black_box(value::get_polymer_length(&input)))
    });
