...
fn main() {
    ...
    println!(
        "Part 2: {:?} with power {}",
        max_square.top, max_square.max_power
    );
}
...
fn get_coords(from: usize, to: usize) -> impl Iterator<Item = (usize, usize)> {
    let xs = (from..=to).cycle().take(to * to);
    let ys = (from..=to).flat_map(move |y| repeat(y).take(to));
...
fn generate_sums(
    powers: &HashMap<(usize, usize), i32>,
    size: usize,
    ...
    for x in 1..=size {
        for y in 1..=size {
            let s: i32 = get((x, y), powers) + get((x - 1, y), &sums) + get((x, y - 1), &sums)
                - get((x - 1, y - 1), &sums);
            sums.insert((x, y), s);
        }
    }
...
fn find_max(size: usize, sums: &HashMap<(usize, usize), i32>, square_size: usize) -> MaxPower {
    ...
    for (x, y) in middle {
        let p: i32 =
            get((x, y), sums) - get((x - square_size, y), sums) - get((x, y - square_size), sums)
    ...
    return MaxPower {
        top: (xm - square_size + 1, ym - square_size + 1, square_size),
        max_power: max,
    };
}

fn find_max_square(size: usize, sums: &HashMap<(usize, usize), i32>) -> MaxPower {
    let mut max_temp = MaxPower {
        max_power: i32::MIN,
        top: (0, 0, 0),
    };
    for i in 1..=size {
