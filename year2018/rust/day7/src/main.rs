use std::{fs, collections::{HashSet, HashMap, BinaryHeap}, cmp::Reverse};

fn main() {
    let file = fs::read_to_string("../data/day7_data.txt")
...
impl Road {
    ...
    fn find_seconds(&self, workers: u32, offset: u32) -> u32 {
        ...
        (0..workers).for_each(|w| _= workers_map.insert(w,Work { node: None, second: 0 }));

        loop {
            workers_map.values_mut()
                .filter(|c| c.node.is_some())
                .for_each(|mut c| {
                    if c.second == 1 {
                        available.insert(c.node.unwrap());
                        *c = Work{node : None, second : 0};
                        return;
                    }
                    c.second -= 1
                });

            let zeros = workers_map.values()
                .filter(|c| c.second == 0)
                .count();
            ...
            for e in evaluate {
                ...
                let zeros = workers_map.iter()
                    .find(|(_, w)| w.node.is_none());
                match zeros {
                    Some((key, _)) => {
                        let seconds = Road::char_seconds(e.0);
                        workers_map.entry(*key)
                            .and_modify(|c| {
                                *c = Work{node: Some(e.0), second: seconds + offset}
                            });
                    },
                    None => break,
                };
...
mod test {
    ...
    fn test_part2() {
        // Arrange
        let file = fs::read_to_string("../data/day7_test_data.txt")
            .expect("couldn't open file");
