...
fn main() {
    let file: String = fs::read_to_string("../../data/day4_data.txt").expect("couldn't open file");
    ...
    for line in &lines_iter {
        let time_and_instruction: Vec<&str> = line.split("] ").collect();

        let (_, _, _, _, minute) = get_date_as_type(line);

        match time_and_instruction[1].chars().next().unwrap() {
            'G' => {
                let index_part: Vec<&str> = time_and_instruction[1].split(" #").collect();
                let index: Vec<&str> = index_part[1].split(" ").collect();
                let id: i32 = index[0].parse().expect("being able to parse");
...
fn find_max_minute(id_max: i32, guard_map: &HashMap<i32, HashMap<i16, i32>>) -> i32 {
    let max_id_entry = guard_map.get(&id_max).expect("should exist");
    let mut max_count = 0;
...
fn get_date_as_type(line: &str) -> (i16, i16, i16, i16, i16) {
    ...
    (
        date_parts[0]
            .parse::<i16>()
            .expect("not able to parse year"),
        date_parts[1]
            .parse::<i16>()
            .expect("not able to parse month"),
        date_parts[2]
            .parse::<i16>()
            .expect("not able to parse date"),
        time_parts[0]
            .trim()
            .parse::<i16>()
            .expect("not able to parse hour"),
        time_parts[1]
            .parse::<i16>()
            .expect("not able to parse minute"),
    )
}
